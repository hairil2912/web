                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="box box-danger color-body" style="min-height: 620px;">
                                    ...
                                    <form class="forms-sample" action="<?= site_url('simpel-aja/pendaftaran/daftar'); ?>" method="POST" id="from_daftar">
                                        <?php if ($config->is_rumkit == '0') : ?>
                                            <div class="box-body">
                                                ...
                                                <select class="form-control" id="id_ruangan" name="id_ruangan">
                                                    <option value=""></option>
                                                    <?php foreach ($poli as $p) : if ($p->is_praktek == '1') : ?>
                                        ...
                                        <?php else : ?>
                                            <div class="box-body">
                                                ...
                                                <div class="form-group">
                                                    <label for="">Cara Bayar<span class="text-danger">*</span></label>
                                                    <select class="form-control" id="id_asuransi_rs" name="id_asuransi">
                                                        <?php foreach ($asuransi as $a) : ?>
                                                            <option value=""></option>
                                                            <option <?= $a->id_asuransi ?> value="<?= $a->id_asuransi ?>"><?= $a->nama_asuransi ?></option>
                                                        <?php endforeach; ?>
                                                    ...
                                                    <label for="">Poli Klinik<span class="text-danger">*</span></label>
                                                    <select class="form-control" id="id_ruangan_rs" name="id_ruangan">
                                                        <option value=""></option>
                                                        <?php foreach ($poli as $p) : if ($p->is_praktek == '1') : ?>
                                                                <option <?= $p->id_ruangan ?> value="<?= $p->id_ruangan ?>"><?= $p->nama_ruangan ?></option>
                                                        <?php endif;
                                                ...
                                                <input type="hidden" name="id_dokter" id="id_dokter_rs">
                                                <div class="form-group" id="keluhan_pasien">
                                                    <label for="">Keluhan</label>
                                                    <textarea class="form-control" id="keluhan" name="keluhan" placeholder="ketik keluhan kk disini" rows="4"></textarea>
                                                </div>
        ...
        if (document.readyState === "complete") {
            ...
            App.save('#from_daftar', function() {}, function(response) {
                if (response.status === true) {
                    $.get(site_url + 'simpel-aja/pendaftaran/success', function(html) {
                        $('#myModal').html(html);
            ...
            $("#id_asuransi, #id_asuransi_rs").select2({
            ...
            $('#id_ruangan, #id_ruangan_rs').select2({
            ...
            $(document).on('change', '#id_asuransi', function() {
                ...
                if (id_asuransi == '101') {
                    $.get('<?= site_url('simpel-aja/pendaftaran/dokter_spesialis_bpjs/') ?>' + id_asuransi, function(response) {
                        $('#id_dokter').html(response)
                    });
                } else {
                    $.get('<?= site_url('simpel-aja/pendaftaran/dokter_spesialis/') ?>' + id_asuransi, function(response) {
                        $('#id_dokter').html(response)
                    });
                }
            });

            $(document).on('change', '#id_ruangan_rs', function() {
                var id_ruangan_rs = $(this).val();
                $.get('<?= site_url('simpel-aja/pendaftaran/dokter/') ?>' + id_ruangan_rs, function(response) {
                    $('#id_dokter_rs').val(response.id_user);
                }, 'json');
            });

...
<main class="main">
	<ol class="breadcrumb">
		...
		<li class="breadcrumb-item"><a href="<?= site_url('admin/event/all') ?>">Events</a></li>
			...
			<form id="form_post" action="<?= current_url() ?>" method="POST">
				<div class="row">
					...
					<div class="form-group">
						...
						<input type="text" name="title" required placeholder="Enter Event Title" class="form-control" value="<?= @$event->title ?>">
					</div>
					<div class="form-group">
						<textarea id="summernote" rows="7" name="content" class="form-control"><?= @$event->content ?></textarea>
					</div>
					</div>
					<div class="col-lg-3">
						<div class="card">
						...
						<div class="card">
							<div class="card-body">
								<p class="box-title">Event Attributes</p>
								<div class="form-group">
									<select id="post_parent" name="post_parent" class="form-control input-sm">
										<option value="">--Parent--</option>
										<?php foreach($event_parent as $parent): ?>
											<option <?= ($parent->post_id == @$event->post_parent) ? 'selected' : '' ?> value="<?= $parent->post_id ?>"><?= $parent->title ?></option>
										<?php endforeach; ?>
									</select>
								</div>
							</div>
						...
						<div class="card">
							...
							<?php else: ?>s
								<input type="hidden" name="featured_image" value="<?= @$event->featured_image ?>">
								<img class="__featured_image" src="<?= base_url('assets/uploads/' . @$event->featured_image) ?>">

...
class SpesialisModel extends CI_Model
{
    ...
    public function get_total_by_type($type)
    {
        ...
        $sql = $this->db->select('p.post_id, p.title, p.content, u.fullname as author, p.modified_on')
                        ->from('klik_posts p')
    ...
    public function all($type, $limit, $offset)
    {
        ...
        $sql = $this->db->select('p.post_id, p.title, p.content, u.fullname as author, p.status, p.modified_on')
                        ->from('klik_posts p')
                        ->join('klik_users u', 'p.author = u.user_id', 'left')
                        ->where([
                            'post_type' => 'spesialis'
    ...
    public function get_page_parent()
    {
        $sql = $this->db->select('post_id, title, content')
                        ->from('klik_posts')
                        ->where('post_type', 'spesialis')

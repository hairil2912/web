...
class Pendaftaran extends PasienController
{
    public function index()
    {
        ...
        $this->template->pasien('pendaftaran/index', $data);
    }
        ...
        if ($_SERVER["REQUEST_METHOD"] === "POST") {
            ...
            if ($this->form_validation->run() == true) {
                $data = [
                    ...
                    'id_ruangan' => (!empty($this->input->post('id_ruangan')) ? $this->input->post('id_ruangan') : null),
                    'id_dokter' => (!empty($this->input->post('id_dokter')) ? $this->input->post('id_dokter') : 0),
    ...
    public function dokter($id_user = null)
    {
        $token = $_COOKIE['token'];
        $curl = curl_init();
        curl_setopt_array($curl, array(
